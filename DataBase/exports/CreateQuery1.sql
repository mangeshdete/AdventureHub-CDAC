...
--
-- Host: localhost    Database: p14_adventurehub
-- ------------------------------------------------------
...
CREATE TABLE `cancelrequests` (
  ...
  `from_status` enum('PROCESSING','TO_BE_CANCELLED') NOT NULL,
  `to_status` enum('ACTIVE','CANCELLED','PROCESSING') DEFAULT NULL,
  `cancellation_reason` text,
  PRIMARY KEY (`id`),
  KEY `publishid` (`publishid`),
  CONSTRAINT `cancelrequests_ibfk_1` FOREIGN KEY (`publishid`) REFERENCES `publishevents` (`publishid`) ON DELETE CASCADE
...
CREATE TABLE `publishevents` (
  ...
  `status` enum('ACTIVE','PROCESSING','CANCELLED','COMPLETED','TO_BE_CANCELLED') NOT NULL,
  `street` varchar(100) NOT NULL,
  `cityid` int NOT NULL,
  `pincode` varchar(6) NOT NULL,
...
-- Dump completed on 2025-02-07 23:57:02
