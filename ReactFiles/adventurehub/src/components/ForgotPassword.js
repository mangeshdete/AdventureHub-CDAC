...
const ForgotPassword = () => {
  ...
  const handleVerifyEmail = () => {
    ...
    fetch(`http://localhost:8140/auth/getUserByEmailId?email=${email}`)
      ...
      .then((isUserExist) => {
        if (isUserExist) {
          ...
          fetch(`http://localhost:8140/auth/getSecurityQuestionByEmailId?email=${email}`)
            .then((response) => response.json())
            .then((data) => {
              setSecurityQuestion(data.question);
  
              // Fetch the security answer
              fetch(`http://localhost:8140/auth/getSecurityAnswerByEmailId?email=${email}`)
                .then((response) => {
  ...
  const handlePasswordReset = () => {
    ...
    fetch('http://localhost:8140/auth/resetPassword', {
      method: 'POST',
